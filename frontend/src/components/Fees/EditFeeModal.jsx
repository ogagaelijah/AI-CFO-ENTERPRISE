// frontend/src/components/Fees/EditFeeModal.jsx
...
const EditFeeModal = ({ isOpen, fee, onSubmit, onClose, error, setError }) => {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      issueDate: toDateInput(fee.issueDate),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {};
      if (form.description !== (fee.description || '')) payload.description = form.description;
      if (form.amount !== '' && Number(form.amount) !== Number(fee.amount)) payload.amount = Number(form.amount);
      if (form.issueDate && form.issueDate !== toDateInput(fee.issueDate)) payload.issueDate = form.issueDate;
      if (form.dueDate !== toDateInput(fee.dueDate)) payload.dueDate = form.dueDate || null;
      if (form.status !== fee.status) payload.status = form.status;
      if (form.notes !== (fee.notes || '')) payload.notes = form.notes;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Edit Fee {fee.feeNumber}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              min="0"
              step="0.01"
              disabled={Number(fee.amountPaid) > 0}
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              DRAFT → SENT creates the debtor row. Other transitions follow the fee rules.
            </p>
          </div>
