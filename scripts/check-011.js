...
(async () => {
    try {
        const cols = await query(
            "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'terms' ORDER BY ordinal_position"
        );
        console.log('--- terms columns ---');
        console.table(cols.rows);

        const idx = await query(
            "SELECT indexname FROM pg_indexes WHERE tablename = 'terms' ORDER BY indexname"
        );
